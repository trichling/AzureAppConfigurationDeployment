...
public class EndToEndTests
{
    ...
    public async Task ExtractKeysAndAddToKeyVault()
    {
        ...
        var configurationSourceKeyExtractor = new ConfigurationKeyExtractor(
            new[] { configurationSource }
        );
        var configurationKeys = configurationSourceKeyExtractor.ExtractKeys();

        var appSettingsSource = new AzureAppSettingsKeySource(
            "apetito:aaa:AzureAppConfigurationDeploymentTests:",
            "",
            new Uri(keyVaultUri)
        );
